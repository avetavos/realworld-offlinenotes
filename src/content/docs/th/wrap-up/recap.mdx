...
| **Thin relay server** | server แค่เก็บและ relay opaque ops — ไม่เคย merge หรือเป็นเจ้าของความจริง ดังนั้นสลับเป็น transport แบบ peer-to-peer ได้โดยไม่แตะ data model | คุณยังต้องรันและ deploy server *ตัวหนึ่ง* อยู่ดี; consistency แบบ "eventual" เป็นเรื่องที่คุณต้องจัดการใน UI | [The Sync Server →](/offlinenotes/th/sync-server/) |
| **Background Sync** | การ push ถูกเลื่อนตอนออฟไลน์และ retry โดย browser เมื่อ connectivity กลับมา ดังนั้น sync จึงไม่เคยบล็อกการแก้ไข | browser support กระท่อนกระแท่น ต้องมี fallback แบบ `online`/focus | [Background Sync →](/offlinenotes/th/background-sync/) |

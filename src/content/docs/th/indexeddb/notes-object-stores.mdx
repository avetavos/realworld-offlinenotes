...
พอจบ `db.ts` export `getNote`, `putNote`, `listNotes`, `getDoc`, `putDoc`, `enqueueOp`, `drainOutbox`, `getMeta` และ `setMeta` module [Notes UI](/offlinenotes/th/notes-ui/) ใช้มันตรง ๆ; sync engine พึ่ง `enqueueOp`/`drainOutbox` ภายหลัง

## Why

- **สี่ store สี่ความรับผิดชอบ** local-first app มีมากกว่า "ข้อมูล": มันมีรูปแบบที่เชื่อถือได้, read model ราคาถูก, queue ขาออก และการจดบัญชีเล็กน้อย การแยกมันทำให้แต่ละการอ่านเร็วและแต่ละการเขียนชัดเจน
...
- **รูปร่างคือ contract** รูปร่าง record เหล่านี้เป๊ะ ๆ ใช้ร่วมกับ CRDT engine, sync server และบทเรียนภาษาไทย การตรึงมันตรงนี้คือสิ่งที่ทำให้ module ต่อ ๆ ไป assume ได้เลย ไม่ต้อง re-derive ว่า record `docs` มีอะไร

แต่ละ store และเหตุผลที่มันมีอยู่:
...
- **`outbox`** — `{ seq, noteId, op }` local op ที่ **ยังไม่ push** ไปยัง sync server key ด้วย `seq` ที่ assign อัตโนมัติ push drain มันตามลำดับ; ไม่มีอะไรตรงนี้ถูกลบจนกว่า server จะ acknowledge
...
- **Cons:** สอง store อธิบาย note เดียวกัน การเขียนจึงต้องอัปเดตทั้งคู่ และมันเพี้ยนได้ถ้า code path ไหนลืม เรากันเรื่องนั้นด้วยการบีบให้ทุก mutation ผ่าน `putNote`/`putDoc`

**store `outbox` เทียบกับการคำนวณ op ที่ยังไม่ sync แบบ on the fly**

- **Pros:** "อะไรยังต้อง push?" เป็น list ถาวรที่รอดจากการ reload และช่วง offline; Background Sync (Module 11) แค่ drain มัน; ลำดับชัดเจนผ่าน `seq`
- **Cons:** มัน duplicate op ที่อยู่ใน snapshot ด้วยจนกว่ามันจะถูก acknowledge และ outbox หดลงเฉพาะตอน push สำเร็จ — outbox ของเครื่องที่ offline โตขึ้นจนกว่ามันจะ reconnect

## Set it up

### 1. `apps/web/src/db.ts` — typed accessors

เพิ่มพวกนี้ต่อยอดจาก `getDb()` ของบทก่อน ทุก function `await` connection ที่ใช้ร่วมกัน มันจึงขี่ upgrade path เดียวกันหมด
...
`actorId` ระบุ replica นี้ให้ CRDT (Module 6) และต้องเสถียรข้ามการ reload เก็บมันใน `meta` ตอนรันครั้งแรก
...
1. ทำไม `notes` ถึงเก็บแค่ `{ id, title, updatedAt }` แทนที่จะเก็บ body เต็มของ note? มันให้อะไรกับ list UI?
2. `deleteNote` ครอบสอง `delete` ไว้ใน transaction เดียว invariant อะไรจะพังถ้ามันทำเป็นสอง `await` แยกกัน?
3. `enqueueOp` เก็บ `{ noteId, op }` แต่ value type ยังระบุ `seq` ด้วย `seq` มาจากไหน และทำไม function ถึง return มัน?

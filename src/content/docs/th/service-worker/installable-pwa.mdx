...
Service Worker ทำให้ OfflineNotes โหลด offline ได้ บทนี้ทำให้มัน **install ได้**: manifest บอก browser ว่านี่คือ app, `beforeinstallprompt` ที่ดักจับไว้ให้เราเสนอ install ในเงื่อนไขของเราเอง, และ DevTools Application panel ยืนยันว่าทุกอย่างทำงานได้โดยปิด network
...
ความ install ได้คือ **contract ที่ browser บังคับใช้**: มันจะเสนอ install เฉพาะ site ที่มี manifest ถูกต้อง (พร้อม field และ icon ที่ต้องมี) *และ* มี Service Worker ที่ควบคุม page อยู่ เราทำครึ่งหลังไปแล้วบทที่แล้ว; manifest คือครึ่งแรก การทำให้เข้า contract ยังเป็น audit ที่มีประโยชน์ด้วย — ถ้า app install ได้ มันก็ offline ได้จริง เพราะ browser ตรวจแล้ว

เราดัก `beforeinstallprompt` แทนที่จะปล่อยให้ browser แสดง default mini-infobar เพราะ **timing และ placement เป็นการตัดสินใจเชิง product** การยิง install prompt ทันทีที่ผู้มาเยือนครั้งแรก landing คือทางที่เร็วที่สุดที่จะโดนปัดทิ้งตลอดกาล การเรียก `preventDefault()` และเก็บ event ไว้ทำให้เราโผล่ install ตรงจุดที่มันสมเหตุสมผล — ข้าง ๆ คำใบ้ว่า "works offline" หลังจาก user ได้เขียน note จริง ๆ แล้ว — และ event ที่เก็บไว้ยัง trigger native prompt *ตัวจริง* ได้เมื่อพวกเขาคลิก

ข้อจำกัดที่ตรงไปตรงมา: `beforeinstallprompt` เป็น feature ของ Chromium และจะไม่ fire ในทุก browser (iOS Safari install ผ่าน Share sheet แทน) และมันจะ fire เฉพาะเมื่อเข้าเกณฑ์และ app ยังไม่ถูก install ดังนั้นปุ่มแบบกำหนดเองคือ progressive enhancement — มีเมื่อ platform เสนอให้ ไม่มีเมื่อมันไม่เสนอ ไม่เคยเป็น dependency ที่ขาดไม่ได้

## Pros & cons

**ดัก `beforeinstallprompt` เพื่อทำปุ่มแบบกำหนดเอง เทียบกับ default prompt ของ browser**

- **Pros:** คุณเลือกได้ว่าจะถามเมื่อไรและตรงไหน ผูกมันกับจังหวะที่ user เห็นคุณค่า และซ่อนมันได้เมื่อ install แล้ว — อัตราตอบรับสูงกว่า banner ที่โผล่มาโดยไม่มีใครขอมาก
...
- **Pros:** มี window และ icon ของตัวเอง ไม่มี address bar เปิดจาก home screen หรือ dock ได้ — มันรู้สึกเหมือน native notes app ที่ project นี้เลียนแบบอยู่
...
link manifest และ set theme color ใน page head เพื่อให้ browser อ่านมันได้
...
ตอนนี้ยืนยันว่ามันเปิดได้โดยไม่มี network ใน DevTools → Network set throttle เป็น **Offline** แล้วเปิด app ที่ install แล้วใหม่ (หรือ reload):
...
ตอนนี้ OfflineNotes เป็น PWA จริงแล้ว: manifest ประกาศว่ามันเป็น app, `beforeinstallprompt` ที่ดักจับไว้เสนอ install ในเงื่อนไขของเรา, และ Application panel ยืนยันว่ามัน install และเปิด offline ได้เต็มรูปแบบ ทั้ง client — data, code, และ shell — รันได้โดยไม่มี network สิ่งที่มันยังทำไม่ได้คือการ *แชร์* edit ระหว่าง device ต่อไป [The Sync Server →](/offlinenotes/th/sync-server/) สร้าง Hono backend แบบบางที่เก็บและ relay CRDT op ที่นั่งอยู่ใน outbox

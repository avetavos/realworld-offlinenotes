...
ตอนนี้ op ไหลออกจากทุก edit แล้ว แต่หายไปตอน reload บทนี้ทำให้ document คงทน: **snapshot** ของ CRDT ทั้งก้อนไปที่ `docs` store, แต่ละ **op** ถูก queue ไว้ใน `outbox`, และการเปิด note ซ้ำจะสร้างขึ้นใหม่ด้วย `NoteDoc.fromSnapshot`
...
`fromSnapshot` คือสิ่งที่ทำให้ snapshot เชื่อถือได้ เพราะสร้าง `NoteDoc` ขึ้นใหม่ด้วย actor id *ตัวเดียวกัน* และ counter ภายในเดิม edit ที่ทำหลัง reload จึงผลิต op ที่เข้าล็อกถูกที่ใน causal history เดียวกัน — note ที่กู้กลับมาแยกไม่ออกจาก note ที่ไม่เคยถูกปิดเลย
...
`openNote` อ่าน `docs`; ถ้ามี record อยู่ ก็สร้างใหม่ด้วย `fromSnapshot` ไม่งั้นเริ่ม `NoteDoc` ใหม่สด ๆ ตอนนี้ edit method จะ `await persist(...)` ก่อน fire `onChange`

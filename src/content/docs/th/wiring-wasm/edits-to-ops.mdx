...
note store (`store.ts`) ที่เป็นเจ้าของ WASM CRDT สำหรับ note ที่เปิดอยู่ มันเปิด edit method ให้สามตัว — `setTitle`, `insertText`, `deleteText` — ที่เรียกเข้า `NoteDoc` เก็บ op ที่ engine ส่งกลับมา แล้ว re-render UI จาก `title()` และ `text()`

การกลับด้านที่เป็นหัวใจ: UI ไม่เคยแก้ string ตรง ๆ มันขอให้ CRDT apply edit และ CRDT คือผู้มีอำนาจตัดสินเพียงหนึ่งเดียวว่าตอนนี้ document พูดว่าอะไร op คือผลพลอยได้ที่เราจะ persist และ sync ในบทถัดไป ตรงนี้เราแค่ทำให้มันไหล
...
การ apply edit ผ่าน `NoteDoc` ก่อนยังให้ op มาฟรี ๆ ด้วย `insert_text` ไม่ได้แค่แก้ document; มัน *return* op ที่อธิบายการแก้นั้น ค่าที่ return มานั้นคือ atom ที่เราเก็บลง outbox แล้ว push ไป server การผลิต state และผลิต sync record ในการเรียกครั้งเดียวหมายความว่าทั้งสองจะไม่มีวันขัดแย้งกันได้
...
- **Pros:** edit เป็นการเรียก method ใน memory ที่ราคาถูก; engine เก็บ index ภายในของมันไว้ให้พร้อมใช้เสมอ
- **Cons:** document ที่มีชีวิตคือ mutable state ที่คุณต้องดูแลอย่างระวัง — หนึ่ง `NoteDoc` ต่อหนึ่ง note ที่เปิดอยู่ และคุณต้องไม่ทำมันหายโดยไม่ persist (บทถัดไปจะปิดช่องว่างนั้น)

## Set it up

### 1. `apps/web/src/store.ts`

boot WASM module แค่ครั้งเดียวเป๊ะ ๆ แล้วเก็บ `NoteDoc` ของ note ที่เปิดอยู่ไว้ใน memory ตัว `init` default export จะ load และ instantiate ไฟล์ `.wasm`; คุณต้อง `await` มันก่อนสร้าง exported type ใด ๆ
...
editor ไม่เคยแก้ string ของตัวเอง มันคำนวณช่วงที่เปลี่ยน เรียก store แล้ววาดกลับสิ่งที่ CRDT รายงานมา snippet นี้แสดง title และการแทนที่ทั้งค่าสำหรับ body — การลดการแทนที่ทั้งก้อนให้เป็น insert/delete ช่วยให้การต่อสายซื่อตรงระหว่างที่เรากำลังเรียนรู้
...
ต่อสาย store เข้ากับ page แล้วขับมันจาก browser console ก่อนอื่นยืนยันว่า WASM package ถูก build แล้ว:

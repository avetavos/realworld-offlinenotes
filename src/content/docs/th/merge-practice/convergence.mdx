---
title: Convergence
description: "Sync ทั้งสอง client แล้วดูว่าตกลงบน text() เดียวกัน, แล้วพิสูจน์ว่าลำดับ op และการซ้ำไม่เปลี่ยนผล — payoff ของ CRDT และต้นทุน tombstone ที่ตามมา"
sidebar:
  order: 2
---

[บทที่แล้ว](/offlinenotes/th/merge-practice/concurrent-edits/) ทิ้งสอง client ไว้แบบแยกทางกัน: หนึ่ง note, สอง `outbox`, edit ที่ server ไม่เคยเห็น ตอนนี้เราปล่อยให้ sync — แล้วเก็บ payoff ที่สถาปัตยกรรมทั้งหมดสร้างมาเพื่อสิ่งนี้ ทั้งสอง client pull op ของกันและกัน, merge ผ่าน [WASM CRDT](/offlinenotes/th/crdt-rust/), แล้วลงเอยที่ **ข้อความเหมือนกันแบบ byte-for-byte** โดยไม่มี prompt "คุณต้องการเวอร์ชันไหน?" ที่ไหนเลย
...
หยิบสอง client offline จากบทที่แล้วมา ในแต่ละ profile ไป DevTools → **Network** → **Online** ตอนนี้แต่ละ client push `outbox` ของตัวเองแล้ว pull op ของอีกฝ่าย (Module 10 / Background Sync) จากนั้น merge:

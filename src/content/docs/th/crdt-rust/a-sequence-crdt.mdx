...
บทนี้เพิ่ม `Rga` เข้า `NoteDoc` และ implement `insert_text`, `delete_text`, `text` และ branch `Insert`/`Delete` ของ `merge` ตามสัญญาเป๊ะ ทุกตัวอักษรได้ id `(counter, actor)` ที่ไม่ซ้ำ; การ insert ตั้งชื่อ element ที่มันวาง *ต่อจาก*; การ delete ทิ้ง **tombstone** ไว้; และกฎ ordering แบบ deterministic ทำให้ทั้งหมด converge ไม่ว่า op จะมาถึงลำดับใด เราจะจบด้วยการพิสูจน์ *ทำไม* มันจึง commutative, associative และ idempotent — และบอกตรง ๆ ว่าเวอร์ชันสร้างมือนี้หยุดตรงไหน
...
RGA ตั้งชื่อทุกตัวอักษรด้วย **id ที่ไม่เปลี่ยน** และบันทึก **ว่ามันถูก insert ต่อจากอะไร** ไม่ใช่ตรงไหน การ insert คือ "วางตัวอักษร `X` ที่มี id `(7, "A")` ต่อจาก element ที่มี id `(3, "B")` ทันที" anchor นั้นไม่เคยเปลี่ยน ดังนั้น op หมายความเหมือนเดิมบนทุก replica ไม่ว่ามันจะมาถึงเมื่อไหร่ สองคน insert ต่อจาก anchor *เดียวกัน* คือการชิงกันจริง ๆ เพียงกรณีเดียว และ RGA ตัดสินมันด้วยกฎตายตัว: ในบรรดา sibling ที่แชร์ anchor **เรียงตาม id จากมากไปน้อย** (id ใหม่กว่าก่อน) เพราะ id ไม่ซ้ำทั่วโลกและ order สมบูรณ์ ทุก replica sort sibling เหล่านั้นเหมือนกัน — ทุกคนจึงจบด้วย string เดียวกัน

การ delete เป็นอีกความละเอียดอ่อน คุณลบ element จริง ๆ ไม่ได้ เพราะ insert ที่พร้อมกันอาจ anchor อยู่กับมัน — ลบมันแล้ว anchor จะห้อยเคว้ง ดังนั้น delete แค่พลิก flag **tombstone**; element ยังอยู่เป็น anchor point แต่หยุดแสดงใน text นั่นคือสิ่งที่ทำให้ delete commute กับทุกอย่าง
...
`merge` จากบทที่แล้วได้เพิ่มสอง arm `Insert` ที่เข้ามาเลื่อน Lamport clock แล้ว integrate element; `Delete` tombstone ด้วย id handler ทั้งคู่ idempotent และผลของมันไม่ขึ้นกับลำดับ — ดังนั้น batch จะมาถึงแบบสลับ, ซ้ำ หรือแทรกกับ local edit ก็ได้ และ body ยัง converge
...
- **Idempotent** — `integrate` เพิกเฉย id ที่มันมีอยู่แล้ว; `tombstone` set flag ที่เป็น `true` อยู่แล้วหรือกำลังจะเป็น การ apply op เดิมสองครั้งเท่ากับ apply ครั้งเดียว ดังนั้นการส่งซ้ำ (sync server *จะ* redeliver) ไม่เป็นอันตราย
- **Commutative** — slot สุดท้ายของ element ถูกตรึงด้วย anchor `after` และ tie-break id-จากมากไปน้อย ซึ่งไม่มีตัวไหนขึ้นกับ *เมื่อไหร่* ที่มัน integrate tombstone แค่พลิก flag ดังนั้นลำดับ op เปลี่ยนผลไม่ได้
- **Associative** — ด้วยเหตุผลเดียวกัน มันไม่สำคัญว่าคุณ *จัดกลุ่ม* การ merge อย่างไร (batch จากเครื่อง B แล้ว C กับ C แล้ว B) ทุกการจัดกลุ่มไปถึง sequence เดียวกัน

รวมกัน: replica ใดก็ตามที่เห็น **set** ของ op เดียวกันแสดง `text()` เดียวกัน นั่นคือ convergence — property ที่ทั้ง architecture ยืนอยู่บนมัน
...
(หมายเหตุเรื่อง ordering: delete ที่มาถึงก่อน insert ที่มันเล็งไว้ หรือ insert ที่มาก่อน anchor ของมัน เป็นไปได้เฉพาะเมื่อ op ถูกส่งผิดลำดับ causal sync server ของเราเก็บ log แบบ **append-only และเรียงลำดับ** ต่อ note ดังนั้น anchor มาก่อน op ที่อ้างถึงมันเสมอ — สมมติฐานที่ fallback ของ `integrate` พึ่งพา CRDT production จะ buffer op ที่มาก่อนไว้จนกว่า dependency ของมันจะมาถึงแทน)

**Check your understanding:**

1. ทำไม RGA anchor การ insert ไว้กับ *สิ่งที่มันมาต่อจาก* แทนที่จะเป็น numeric index?
2. สองเครื่อง insert ตัวอักษรต่างกันต่อจาก anchor เดียวกัน กฎอะไรตัดสินลำดับ และทำไมทุก replica เห็นด้วยกับมัน?
3. ทำไมการ delete จึงทิ้ง tombstone ไว้แทนที่จะลบ element ทิ้งเลย?
4. บอกชื่อต้นทุนเฉพาะที่ RGA สร้างมือนี้จ่าย ซึ่ง Automerge/Yjs เลี่ยงได้ และอะไรแก้มัน
...
CRDT engine ครบแล้ว — `NoteDoc` เปลี่ยน edit เป็น op และ merge op กลับ ต่อไปเราต่อสายมันเข้า note store ให้ local edit ไหลผ่าน WASM core เข้า IndexedDB: [Wiring the WASM Core →](/offlinenotes/th/wiring-wasm/)

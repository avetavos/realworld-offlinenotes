...
`NoteDoc` มีแค่ `title` ก่อนบทนี้ ต้องเพิ่ม field `body: Rga` ที่ทั้งบทนี้อ้างอิงถึง และ initialize มันใน constructor ต่อจาก field ที่มาจากบท [LWW register](/offlinenotes/th/crdt-rust/lww-register/)
...
นี่คือหัวใจของ RGA หา position ของ anchor แล้วเดินผ่าน sibling ที่แชร์ anchor นั้นซึ่งมี id **มากกว่า** ตัวใหม่ (กฎ id-จากมากไปน้อย) แล้ว insert การ integrate id ที่เรามีอยู่แล้วเป็น no-op — นั่นคือ **idempotence**

การข้าม sibling ที่ id สูงกว่าเฉย ๆ ยังไม่พอ — ต้องข้าม **ทั้ง subtree** ของมันด้วย คือทุก element ที่ chain ต่อจากมันไปแล้ว ไม่ใช่แค่ตัวอักษรเดียวนั้น ถ้าหยุดแค่ที่ตัว sibling เอง run หลายตัวอักษรที่มาทีหลังจะ interleave ทีละตัวกับ run ก่อนหน้าแทนที่จะอยู่รวมกันเป็นก้อนเดียว แล้วสอง replica ที่ interleave ในลำดับต่างกันก็จะได้ผลไม่ตรงกัน — นี่คือ "interleaving anomaly" ที่ RGA แบบไร้เดียงสาขึ้นชื่อ `descends_from` ตอบคำถามนี้ด้วยการเดินตาม pointer `after` ย้อนขึ้นจาก candidate จนกว่าจะถึง anchor ของเรา (แปลว่ามันซ้อนอยู่ใต้ sibling ที่กำลังข้าม — เดินต่อ) หรือหลุดออกไปนอกจุดเริ่มต้นเอกสาร (ไม่เกี่ยวข้อง — หยุด) เมื่อมีสิ่งนี้แล้ว การวางขึ้นกับแค่ anchor และการเทียบ id ไม่เคยขึ้นกับลำดับที่มาถึง สอง replica ที่ integrate element ชุดเดียวกันจึงลงที่ sequence เดียวกัน — นั่นคือ **commutativity และ associativity**
...
test สำคัญสุด: สอง replica insert เข้า body เดียวกัน (เริ่มว่าง) พร้อมกัน, แลก op และต้องแสดง text เหมือนกันบนทั้งคู่ — และการ re-merge op เดิมต้องไม่เปลี่ยนอะไร (idempotence) เป็นคนละ module ชื่อ `rga_tests` เพื่อไม่ให้ชนกับ module `tests` ที่บท [LWW register](/offlinenotes/th/crdt-rust/lww-register/) เพิ่มเข้า `lib.rs` ไปแล้ว เพิ่มโค้ดนี้แล้วรัน test:

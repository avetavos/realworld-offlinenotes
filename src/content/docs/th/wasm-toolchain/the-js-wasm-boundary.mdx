...
[บทก่อน](/offlinenotes/th/wasm-toolchain/wasm-bindgen-basics/) ส่งแค่ number กับ string ข้าม boundary CRDT ต้องยื่น **op** ทั้งก้อนให้ JavaScript — record แบบ tagged เช่น insert หรือ delete — และรับ batch ของมันกลับมา merge บทนี้ต่อสาย `serde-wasm-bindgen` เพื่อให้ Rust `enum` กลายเป็น JS object ธรรมดาและกลับมาได้ และปักหมุดชัด ๆ ว่าอะไร copy ข้ามเส้น และใครรับผิดชอบ free อะไร

เราจะนิยาม type `Op` ที่แอปทั้งหมดสร้างขึ้นรอบ ๆ — shape เดียวกับที่เก็บใน IndexedDB และ push ไป sync server — แล้วพิสูจน์ว่ามัน round-trip ได้ CRDT ที่ *ผลิต* op เหล่านี้จะมาใน module ถัดไป; ที่นี่เราทำ wire format ให้แน่นก่อน

## Why

wasm-bindgen marshal primitive และ string ได้เอง แต่ Rust struct หรือ enum ไม่มี representation ที่เป็นธรรมชาติใน JavaScript — memory layout ของมันเป็นเรื่องของ Rust คุณมีสองทางข้าม: serialize เป็น JSON **string** แล้ว `JSON.parse` อีกฝั่ง หรือ convert ตรงเป็น **JS value** (object, array, number) โดยไม่มี string คั่นกลาง `serde-wasm-bindgen` ทำอย่างหลัง และมันถูกต้อง: ไม่มี double-encoding, โค้ดเล็กกว่า และผลลัพธ์เป็น JS object ที่ note store ตรวจได้ทันที
...
อีกครึ่งคือ ownership Rust struct ที่ export ไป JS (เช่น `NoteDoc`) อยู่บน WASM heap และเข้าถึงผ่าน pointer ที่ JS object ถือ; **op ต่างออกไป** — มันเป็น *copy* ที่ถูก serialize เป็น JS data ธรรมดาที่ไม่มี Rust lifetime ผูกอยู่ การรู้ว่าอันไหนเป็นอันไหนบอกคุณว่าเมื่อไหร่ต้องเรียก `.free()` และเมื่อไหร่ garbage collector จัดการให้แล้ว
...
- **Cons of copying:** ทุกการข้าม serialize และ allocate สำหรับ batch ใหญ่นั่นเป็นงานจริง — เราจึงข้าม boundary เป็น **batch** (array ของ op ทั้งชุดต่อ call) ไม่ใช่ทีละตัวอักษร เพื่อ amortize มัน
...
element id กับ timestamp ต่างเป็น pair `(counter, actor)` — Rust tuple ซึ่ง serde serialize เป็น JS array สองสมาชิก `` `[counter, actor]` `` เรา alias มันเพื่อให้อ่านง่าย `#[serde(tag = "t")]` และ `rename` ต่อ variant ผลิต wire shape เป๊ะที่คอร์สที่เหลือพึ่งพา
...
object ที่คุณได้จาก `demo_ops()` เป็น JS ธรรมดาที่คุณ log, store หรือส่งได้ ยื่นมันกลับให้ `count_ops` ตรง ๆ แล้วมัน deserialize สะอาด เพราะทั้งสองทิศอ่านนิยาม serde เดียวกัน

...
ใน [Push & Pull Sync](/offlinenotes/th/push-pull/) client drain `outbox` ของมันทันทีที่ edit ลงมา — แต่เฉพาะเมื่อมี network ตอน offline การ push ล้มเหลว op ค้างอยู่ใน queue และ *อะไรสักอย่าง* ต้องจำไว้ว่าจะลองใหม่ทีหลัง จนถึงตอนนี้ "อะไรสักอย่าง" นั้นคือ user เปิด app ขึ้นมาใหม่ บทนี้ยกงานนั้นให้ browser
...
client helper เล็ก ๆ ชื่อ `requestSync()` ที่ขอให้ browser schedule background sync ภายใต้ tag `sync-notes` บวกกับหนึ่งบรรทัดใน write path ของ store ที่เรียกมันเมื่อ push ตอนนี้ทำไม่ได้ ส่วนฝั่ง Service Worker — ที่ drain `outbox` จริง ๆ — คือ[บทถัดไป](/offlinenotes/th/background-sync/retry-when-online/) ตรงนี้เราแค่ *ตั้ง* การ retry ไว้

## Why

edit ที่ทำ offline persist ลง IndexedDB อย่างปลอดภัยแล้วผ่าน [WASM CRDT](/offlinenotes/th/crdt-rust/); note ไม่ได้เสี่ยงอะไร สิ่งที่ขาดคือการส่งมอบ: op ที่ค้างใน queue ต้องไปถึง server ในที่สุด การ poll เปลือง battery และก็ยังพลาดจังหวะ "คุณกลับมา online แล้ว" ได้ถึงหนึ่งช่วง poll interval Background Sync พลิกมัน — browser รู้อยู่แล้วว่าวินาทีไหนที่ radio กลับมาพอดี ก็ให้มันปลุกเราตอนนั้น การ register tag นั้นถูกและ idempotent: ขอบ่อยแค่ไหนก็ได้; browser รวม tag ซ้ำ ๆ ให้เป็น pending sync เดียว

## Pros & cons

**Background Sync tag vs. pushing on every edit**

- **Pros:** browser เป็นเจ้าของการ retry — มัน fire เมื่อ connectivity มีจริง อยู่รอดแม้ปิด tab และ back off เองอัตโนมัติเมื่อล้มเหลวซ้ำ ๆ ไม่มี timer ไม่มี logic reconnect ใน app คุณ
- **Cons:** ตอนนี้มันมีแค่บน Chromium (ไม่มีใน Firefox ไม่มีใน Safari) จึงต้องมองมันเป็น enhancement ที่มี fallback ไม่ใช่ path เดียว fallback นั้นคือบทถัดไป
...
store เรียก `enqueueOp` หลังทุก local edit อยู่แล้ว (Module 10) เพิ่มทางแยก: push ตอนนี้ถ้าเรา online ไม่งั้นเลื่อนไปให้ Background Sync การทิ้ง op ไว้ใน `outbox` ถูกต้องพอดี — มันคือ payload ของการ retry
...
สังเกตลำดับ: `enqueueOp` รัน *ก่อน* ความพยายามส่งมอบทั้งสองแบบ op คงทนอยู่ใน IndexedDB ไม่ว่า network จะทำอะไรต่อ — การเลื่อน sync ไม่เคยเสี่ยงต่อ edit เสี่ยงแค่การส่งมอบมันเท่านั้น
...
4. `registration.sync.register()` throw ได้ ยกหนึ่งเงื่อนไขที่มัน throw และอธิบายว่าทำไมการกลืนมันไว้ (return `false`) จึงเป็นทางเลือกที่ถูกตรงนี้

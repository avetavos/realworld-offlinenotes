...
server เป็น **thin relay**: มันไม่เคย merge และไม่เคยเป็นเจ้าของความจริง (CRDT และ client ทำสิ่งนั้น) แต่มันเป็นเจ้าของสิ่งหนึ่งที่ต้องรอดจากการ restart — **op log แบบ append-only ต่อ note** พร้อม **server sequence number ที่ monotonic** เพื่อให้ `?since=` ส่ง ops ที่อุปกรณ์ยังไม่เห็นให้ได้ตรงเป๊ะ `Map` ให้ seq นั้นมาฟรีใน process เดียว แต่บน edge ไม่มี process เดี่ยว เราจึงต้องการ storage ที่ทั้ง **durable** และ **ถูกต้องภายใต้การ push พร้อมกัน**
...
- **Durable Object — Pros:** DO คือ actor แบบ single-threaded ตัวเดียวที่ addressable ได้ `idFromName(noteId)` ให้ object หนึ่งตัวต่อ note พอดี และ input gate ของมัน serialize request ดังนั้นการเพิ่ม seq และ append op จึง **atomic โดยไม่ต้องเขียน lock เอง** storage ของมัน strongly consistent นี่คือตัวที่เข้ากับ append log ต่อ key แบบในตำรา **Cons:** มีอีกหนึ่ง concept (class + migration) และ note ที่เป็น global hotspot จะ route ผ่าน object ตัวเดียว
...
- **Cons:** สอง deploy และ CORS config ที่ต้องดูแล เราแยกมันไว้เพราะ architecture มอง server เป็น transport ที่สลับได้ — มันอาจกลายเป็น peer-to-peer ทีหลัง และ client ไม่ควรต้องแคร์
...
หนึ่ง object ต่อ note มันถือ counter `head` และทุก op โดย key ด้วย seq ที่ zero-pad ไว้ เพื่อให้ลำดับ lexical เท่ากับลำดับ numeric — ซึ่งทำให้ "ops ตั้งแต่ N" เป็น prefix range read ครั้งเดียว ทั้งสอง method รันภายใน input gate ของ DO ดังนั้น seq จึง race ไม่ได้
...
DO ต้องการทั้ง binding *และ* migration ที่ register class นั้น `new_sqlite_classes` ใช้ storage แบบ SQLite ปัจจุบัน ซึ่งเป็นตัวที่ `this.ctx.storage` read และ write
...
ถ้าคุณอยากส่ง KV มากกว่า ให้สร้าง namespace แล้ว bind มัน — แต่อ่านข้อแม้ก่อน
...
ฝั่ง read จะ list key ใต้ prefix `op:` ของ note นั้นแล้ว filter ด้วย seq มันทำงานได้ — แต่อุปกรณ์สองเครื่องที่ push พร้อมกันอาจ read `head` ตัวเดียวกันทั้งคู่ แล้วตัวหนึ่งทับ seq ของอีกตัว นั่นคือต้นทุนของ eventual-consistency แบบไม่มี atomic increment Durable Object มีอยู่เพื่อปิดช่องนั้นพอดี
...
`PUBLIC_SYNC_URL` เป็น public env var แบบ build-time ที่ client อ่าน (ค่าเริ่มต้น `http://localhost:8787`) rebuild แล้ว redeploy PWA โดยตั้งค่ามันเป็น URL ของ Worker:
...
3. **End to end:** เปิด PWA ที่ deploy แล้วในเบราว์เซอร์สองตัว (หรือสองอุปกรณ์) แก้ไข note แบบออฟไลน์ในแต่ละตัว เอาทั้งคู่ออนไลน์ แล้วยืนยันว่าแต่ละอุปกรณ์ pull ops ของอีกฝั่งมา และ text ลู่เข้าหากัน — ผลตอบแทนจาก Module 12 คราวนี้บน network จริง
...
1. ทำไม op log แบบ `Map` ใน memory จึงทำงานบน `localhost` แต่พังทันทีที่มันเป็น Worker?
2. server ไม่เคย merge ops — แล้วความถูกต้องชิ้นเดียวที่มันต้องรับประกันคืออะไร และ store ตัวไหนรับประกันมันให้ฟรี?
3. ทำไมการ push พร้อมกันสองครั้งจึง race seq counter บน KV ได้ แต่บน Durable Object ไม่ได้?
4. `PUBLIC_SYNC_URL` ถูก bake เข้าไปตอน build อะไรต้องเกิดขึ้นเพื่อให้ client เริ่มคุยกับ server ที่ deploy แล้ว และทำไมมันจึงไม่ใช่ค่า runtime?

## Recap

คุณย้าย sync server ออกจาก `localhost`: Hono Worker ที่ relay ops โดยมี Durable Object ต่อ note หนุนหลัง ซึ่งทำให้ append-only log durable และ seq ของมัน monotonic ภายใต้ concurrency — พร้อมโชว์ KV เป็นทางเลือกที่ง่ายกว่าและตั้งชื่อ race ของมันอย่างซื่อตรง เมื่อ `PUBLIC_SYNC_URL` ถูกชี้ใหม่ อุปกรณ์จริงสองเครื่องก็ sync กันได้แบบ conflict-free บน edge แล้ว OfflineNotes deploy ครบสมบูรณ์แล้ว

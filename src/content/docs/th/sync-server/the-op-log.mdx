...
server `apps/sync` — backend ทั้งหมดของ OfflineNotes บางอย่างตั้งใจ: **เก็บและ relay** CRDT op ของแต่ละ note แล้วไม่ทำอะไรอย่างอื่นเลย ไม่เคย merge, ไม่เคย validate เนื้อหา note, ไม่เคยเป็นเจ้าของความจริง client ยังคงเป็นแหล่งความจริง (ดู [Wiring the WASM Core →](/offlinenotes/th/wiring-wasm/)); server ตัวนี้เป็นแค่ตู้จดหมายที่ device หย่อน op ลงไปและหยิบ op ออกมา
...
- **Cons:** op ทุกตัวหายตอน restart และ scale ไม่ได้เกินหนึ่ง process เรายอมรับข้อนี้สำหรับ dev แล้วเปลี่ยนไปใช้ durable store ใน [Deployment →](/offlinenotes/th/deployment/)

...
สอง route บน Hono server จาก[บทที่แล้ว](/offlinenotes/th/sync-server/the-op-log/) และ **wire format** ที่พวกมันพูด:
...
สอง shape นี้ asymmetric อย่างตั้งใจ ตอน **push** client ถือ op อยู่แล้ว (มันออกมาจาก WASM engine และนั่งอยู่ใน outbox) มันจึงส่ง op เปล่า ๆ และแค่ต้องรู้ว่า server รับมันแล้ว — reply จึงเป็นแค่ `{ head }`, ปลายใหม่ของ log ตอน **pull** client ต้องการทั้ง op *และ* cursor เพื่อจำว่าหยุดตรงไหน แต่ละ op จึงถูกห่อมาเป็น `{ seq, op }` และ response ทวน `head` ซ้ำ device เก็บ `seq` ตัวมากที่สุดที่มันเห็นแล้วส่งกลับเป็น `?since=` ครั้งถัดไป

`since` เป็นแบบ exclusive: `?since=5` return op ที่ `seq > 5` device ที่เพิ่งเกิดใหม่ pull ด้วย `?since=0` แล้วได้ประวัติทั้งหมด นี่คือสิ่งที่ทำให้ pull **resumable และ idempotent** — pull สองครั้งด้วย cursor เดียวกันแล้วการเรียกครั้งที่สอง return op ชุดเดิม (หรือไม่มีเลย) และเพราะ CRDT merge เป็น idempotent การ re-merge มันจึงไม่เปลี่ยนอะไร
...
- **Cons:** cursor เป็นแบบต่อ note และต่อ server; ชี้ client ไปที่ server ใหม่แล้วมันต้อง re-pull ตั้งแต่ `0` ยอมรับได้สำหรับ relay ที่ store ทิ้งได้

**return `{ seq, op }` เทียบกับ return op เปล่าตอน pull**

- **Pros:** client ได้ cursor มาแนบกับ data เลย มันจึงไม่ต้องเดาว่าหยุดตรงไหน — แค่ track `seq` ตัวมากที่สุด
- **Cons:** เปลืองไม่กี่ byte ต่อ op เทียบกับ array เปล่าของ push เล็กน้อยเมื่อเทียบกับ payload ของ op และมันกำจัด bug ทั้งชนิด "ฉันพลาดไปตัวหนึ่งหรือเปล่า?" ออกไป

## Set it up

### 1. `apps/sync/src/index.ts`

เพิ่มสอง route เข้า app เหนือ `serve(...)` พวกมันพึ่ง `append` / `since` / `head` จาก `log.ts` ทั้งหมด — handler ไม่มี logic ของตัวเองเลย
...
สังเกต `c.req.param('id')` สำหรับ path segment, `c.req.query('since')` สำหรับ query string (coerce ด้วย `Number` โดย default เป็น `0`) และ `await c.req.json<...>()` สำหรับ body server ไม่เคยตรวจ op — มันส่ง array `ops` ทั้งก้อนให้ `append` โดยไม่แตะ
...
คาดหวัง — ทั้งสอง op แต่ละตัวมี `seq` ของมัน:
...
1. ทำไม push ถึง return แค่ `{ head }` ในขณะที่ pull ห่อแต่ละ op เป็น `{ seq, op }`?
2. `?since=0` return อะไร และเมื่อไรที่ device จะส่งมัน?
3. `since` เป็น exclusive อะไรจะพังถ้า device เก็บ cursor ผิดแล้ว re-pull op ที่มันมีอยู่แล้ว — และทำไม CRDT ถึงทำให้เรื่องนั้นไม่มีผลร้าย?

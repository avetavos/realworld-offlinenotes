...
description: "Save the CRDT snapshot to the docs store, queue each op in the outbox, and restore a note with NoteDoc.from_snapshot so a reload loses nothing."
sidebar:
  order: 2
---

Ops now flow from every edit, but they vanish on reload. This lesson makes the document durable: the full CRDT **snapshot** goes to the `docs` store, each **op** is queued in the `outbox`, and reopening a note rebuilds it with `NoteDoc.from_snapshot`.
...
- **On open** — read the note's record from `docs` and reconstruct the live document with `NoteDoc.from_snapshot(actorId, snapshot)`.
...
`from_snapshot` is what makes the snapshot trustworthy. Because it reconstructs a `NoteDoc` with the *same* actor id and internal counters, edits made after a reload produce ops that slot correctly into the same causal history — a restored note is indistinguishable from one that was never closed.

## Pros & cons

**Persisting a snapshot on every edit, vs. persisting only the op log.**

- **Pros:** Opening a note is one `getDoc` and one `from_snapshot` — constant time, no replay. Survives reload and browser restart.
...
### 2. `crates/crdt/src/lib.rs` — `snapshot` and `from_snapshot`

The CRDT module never gave `NoteDoc` a way to serialize or rebuild itself — add it now, since this is the first lesson that needs it. `docs` stores whatever `snapshot()` returns and hands it back to `from_snapshot` unchanged, so the wire shape only has to make sense to Rust. Reconstructing the Lamport counter as the max timestamp seen in either the title or the body means the very next local edit after a reload still outranks everything already in the document.
...
Two tiny accessors feed `from_snapshot`'s counter math — add them next to `Lww::merge` and `Rga::tombstone` respectively:
...
Rebuild the WASM package so `snapshot`/`from_snapshot` land in `pkg/`:
...
`openNote` reads `docs`; if a record exists it rebuilds with `from_snapshot`, otherwise it starts a fresh `NoteDoc`. The edit methods now `await persist(...)` before firing `onChange`.
...
2. What does `NoteDoc.from_snapshot(actorId, snapshot)` reconstruct that a fresh `new NoteDoc(actorId)` would not have?
...
A note is now durable: every edit saves its snapshot to `docs`, queues its ops in `outbox`, and refreshes the `notes` projection; reopening rebuilds the exact document with `from_snapshot`. The outbox is a precise record of what we still owe the network — but nothing reads it yet, and the app still needs a live server to work. Next, [Service Worker (Offline) →](/offlinenotes/en/service-worker/) makes the app shell itself load with no network, so OfflineNotes truly runs offline.
